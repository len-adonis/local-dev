# Generated by Powerlevel10k configuration wizard on 2026-09-03 at 13:42 EDT.
...
() {
  ...
  typeset -g POWERLEVEL9K_VCS_COMMIT_ICON='\UEAFC '
  ...
  typeset -g POWERLEVEL9K_AWS_ICON_BEFORE_CONTENT=true
  ...
  #######################[ jj: current Jujutsu change ]########################

  function prompt_jj() {
    local full unique shown suffix

    read -r full unique <<< "$(
      command jj log --ignore-working-copy -r @ --no-graph \
        -T 'change_id ++ " " ++ change_id.shortest()' 2>/dev/null
    )" || return

    [[ -n "$full" && -n "$unique" ]] || return

    shown="${full[1,8]}"
    suffix="${shown#$unique}"

    p10k segment -f 109 -i $'\uEF01' \
      -t "%B${unique}%b%F{245}${suffix}%f"
  }
  ...
  typeset -g POWERLEVEL9K_CUSTOM_MISE_DEFAULT_FOREGROUND=7
  typeset -g POWERLEVEL9K_CUSTOM_MISE_DEFAULT_BACKGROUND=235
